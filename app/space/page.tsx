...
function OrbitRings() {
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
      {[280, 400, 540].map((size, i) => (
        <motion.div
          ...
          style={{
            width: size,
            height: size,
            borderColor: `rgba(6, 182, 212, ${0.08 - i * 0.02})`,
            borderWidth: 1,
          }}
          animate={{ rotate: 360 }}
        ...
        >
          ...
          <motion.div
            ...
            style={{
              width: 4 + i * 2,
              height: 4 + i * 2,
              background: i === 0 ? "#06b6d4" : i === 1 ? "#a855f7" : "#f59e0b",
              top: -2 - i,
              left: "50%",
              marginLeft: -2 - i,
              boxShadow: `0 0 ${8 + i * 4}px ${i === 0 ? "#06b6d4" : i === 1 ? "#a855f7" : "#f59e0b"}`,
            }}
          />
        </motion.div>
      ))}
      ...
      <div className="absolute w-32 h-32 rounded-full bg-cyan-400/5 blur-3xl" />
    </div>
  );
}
...
function MissionClock() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      // ISS Expedition days (rough estimate from a known date)
  ...
  return (
    <div className="flex items-center gap-6 text-xs font-mono">
      ...
      <div>
        <span className="text-white/40">Mission Day </span>
        <span className="text-purple-400">{missionDay}</span>
      </div>
    </div>
  );
}

/* ═══ NEO (Near-Earth Objects) Widget — Enhanced ═══ */
function NEOWidget() {
          ...
          {neos.map((neo: any, i: number) => {
            ...
            return (
              <motion.div key={neo.id || i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                ...
                className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-all group">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white/80 truncate font-medium">{neo.name?.replace(/[()]/g, '')}</p>
                  <div className="flex items-center gap-3 mt-0.5">
                    {diameter && (
                      <span className="text-[10px] text-white/30 font-mono">⌀ {Math.round(diameter)}m</span>
                    )}
                    {dist && (
                      <span className="text-[10px] text-white/30 font-mono">{Number(Number(dist).toFixed(0)).toLocaleString()} km</span>
                    )}
                    {velocity && (
                      <span className="text-[10px] text-white/30 font-mono">{Number(Number(velocity).toFixed(0)).toLocaleString()} km/h</span>
                    )}
                  </div>
                </div>
...
function SpaceXLaunch() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const diff = new Date(launch.date_utc).getTime() - Date.now();
  ...
  if (loading) return <div className="glass-card p-6 animate-pulse"><div className="h-40 bg-white/5 rounded" /></div>;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-[10px] text-purple-400 font-mono">
          SpaceX
        </span>
      </div>

      <div className="mb-4">
        <h4 className="text-white font-semibold">{launch.name}</h4>
        <p className="text-white/40 text-xs font-mono mt-1">
          {new Date(launch.date_utc).toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
        </p>
      </div>

      {/* Countdown */}
      <div className="p-4 rounded-xl bg-gradient-to-r from-purple-500/5 to-cyan-500/5 border border-white/[0.06]">
        <div className="text-[10px] text-white/40 font-mono uppercase tracking-wider mb-1">T-minus</div>
      ...
      {launch.details && (
        <p className="text-white/40 text-xs mt-4 leading-relaxed line-clamp-2">{launch.details}</p>
      )}
    </motion.div>
  );
}

/* ═══ Solar System Quick Cards ═══ */
const PLANETS = [
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {PLANETS.map((p, i) => (
          <motion.div
            ...
            className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.06] text-center transition-all cursor-default group"
          >
...
function SpaceWeather() {
  ...
  useEffect(() => {
    // DONKI API for solar flares & geomagnetic storms
        ...
        <div className="p-3 rounded-lg bg-amber-500/5 border border-amber-500/10 mb-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-white/30 font-mono">
              {new Date(latestFlare.beginTime).toLocaleDateString()}
            </span>
          </div>
        </div>
      )}
      ...
      {kpIndex !== undefined && (
        <div className="p-3 rounded-lg bg-purple-500/5 border border-purple-500/10">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-white/30 font-mono">
              {Number(kpIndex) >= 5 ? "Storm conditions" : "Quiet"}
            </span>
          </div>
          {/* Kp bar */}
          <div className="mt-2 h-1.5 bg-white/5 rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(Number(kpIndex) / 9) * 100}%` }}
              transition={{ duration: 1, ease: "easeOut" }}
              className="h-full rounded-full"
              style={{
                background: Number(kpIndex) >= 7 ? "#ef4444" : Number(kpIndex) >= 5 ? "#f59e0b" : "#22c55e",
              }}
            />
          </div>
        </div>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card overflow-hidden">
      ...
      <div className="relative" style={{ height: 340 }}>
        <iframe
          ...
          title="ISS Live Tracker Map"
          loading="lazy"
          allow="fullscreen"
        />
        <div className="absolute bottom-2 right-2 bg-black/70 backdrop-blur-sm text-[9px] text-white/40 font-mono px-2 py-1 rounded">
          ESA ISS Tracker
        </div>
      </div>
    </motion.div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/20 text-[10px] text-red-400 font-mono">
          {photos[0]?.rover?.name}
        </span>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  onClick={() => setSelectedPhoto(null)}
                  className="text-white/60 hover:text-white text-sm font-mono px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 transition-all"
                >
                  Close
                </button>
              </div>
...
function StatCard({ label, value, icon, color, sub }: { label: string; value: string; icon: string; color: string; sub?: string }) {
  return (
    <motion.div
      ...
      whileInView={{ opacity: 1, scale: 1 }}
      viewport={{ once: true }}
      whileHover={{ scale: 1.02, borderColor: `${color}33` }}
    ...
    <>
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 text-center px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 1 }}>
            ...
            <p className="mt-6 text-white/50 max-w-xl mx-auto text-lg leading-relaxed">
              Real-time telemetry from orbit, solar weather, Mars imagery, and asteroid tracking — your personal mission control.
            </p>
          </motion.div>
      ...
      <main className="px-4 sm:px-6 pb-24 mt-12">
        <div className="max-w-[1400px] mx-auto space-y-8">
          ...
          {/* NASA + Space Weather Row */}
          <Sec delay={0.15}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-0.5 bg-gradient-to-r from-purple-400 to-transparent" />
              <h2 className="text-xs font-mono uppercase tracking-[0.3em] text-purple-400/70">NASA &amp; Solar Activity</h2>
            </div>
          ...
          {/* Near-Earth Objects + SpaceX Launch */}
          <Sec delay={0.2}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-0.5 bg-gradient-to-r from-amber-400 to-transparent" />
              <h2 className="text-xs font-mono uppercase tracking-[0.3em] text-amber-400/70">Asteroid Watch &amp; Launches</h2>
            </div>
          ...
          <Sec delay={0.25}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xs font-mono uppercase tracking-[0.3em] text-red-400/70">Mars Surface</h2>
            </div>
            <MarsRoverPhotos />
          </Sec>
          ...
          <Sec delay={0.3}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-0.5 bg-gradient-to-r from-indigo-400 to-transparent" />
      ...
      <footer className="py-12 px-4 sm:px-6 border-t border-white/5">
        <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-white/20 text-[10px] font-mono">Data from NASA, ESA, SpaceX</p>
          </div>
